...
{
    public class UnapplyMaxHpUpVisualsSystem : ReactiveSystem<GameEntity>
    {
        public UnapplyMaxHpUpVisualsSystem(GameContext gameContext) : base(gameContext)
        {
        ...
        protected override void Execute(List<GameEntity> statuses)
        {
            foreach (GameEntity status in statuses)
            {
                ...
                if (target is {hasStatusVisuals: true}) 
                    target.StatusVisuals.UnapplyMaxHp();
            }
        }
    }
}